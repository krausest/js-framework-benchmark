        ...
        if (fs.existsSync(file)) {
          ...
          for (let key of Object.keys(data.values)) {
            ...
            if (
              ...
              console.log(
                `WARNING: for ${framework.uri} and benchmark ${benchmarkInfo.id} count was ${
                  vals.length
                }. We expected ${config.NUM_ITERATIONS_FOR_BENCHMARK_CPU + benchmarkInfo.additionalNumberOfRuns}`
              );
            } else if (
