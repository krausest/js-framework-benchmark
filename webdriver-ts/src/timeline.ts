...
interface Timingresult {
  ...
  export function extractRelevantEvents(entries: any[]) {
    ...
    entries.forEach(x => {
        ...
        //     filteredEvents.push({type:'compositelayers', ts: +e.ts, dur: +e.dur, end: +e.ts+e.dur, pid: e.pid, evt: JSON.stringify(e)});
        ...
        //   if (config.LOG_DETAILS) console.log("RunTask",+e.ts, +e.ts+e.dur-click_start);
        //     filteredEvents.push({type:'runtask', ts: +e.ts, dur: +e.dur, end: +e.ts+e.dur, pid: e.pid, evt: JSON.stringify(e)});
        ...
        // } else if (e.name==='Layerize' && e.ph==="X") {
        //   if (config.LOG_DETAILS) console.log("LAYERIZE",+e.ts, +e.ts+e.dur-click_start);
        //     filteredEvents.push({type:'layerize', ts: +e.ts, dur: +e.dur, end: +e.ts+e.dur, pid: e.pid, evt: JSON.stringify(e)});
        ...
        //     filteredEvents.push({type:'updateLayoutTree', ts: +e.ts, dur: +e.dur, end: +e.ts+e.dur, pid: e.pid, evt: JSON.stringify(e)});
  ...
  export async function computeResultsCPU(fileName: string, warning_logger: (...msg:any) => void = console.log): Promise<CPUDurationResult> {
    ...
    // let rafs_withinClick = R.filter((e: Timingresult) => e.ts >= click.ts && e.ts <= click.end)(R.filter(type_eq('requestAnimationFrame'))(eventsOnMainThreadDuringBenchmark));
    // let fafs =  R.filter((e: Timingresult) => e.ts >= click.ts && e.ts < commit.ts)(R.filter(type_eq('fireAnimationFrame'))(eventsOnMainThreadDuringBenchmark));
    ...
    return {tsStart:click.ts, tsEnd:commit.end,  duration, droppedNonMainProcessCommitEvents, droppedNonMainProcessOtherEvents, maxDeltaBetweenCommits, numberCommits};
  }
  ...
  export class PlausibilityCheck {
    moreThanOneCommit = new Set<string>();
    layoutForSelectRow = new Set<string>();
    ...
    check(result: CPUDurationResult, trace: string, framework: FrameworkData, benchmarkInfo: CPUBenchmarkInfo) {
      // if (!benchmarkInfo.layoutEventRequired && result.numberofLayouts > 0) {
      //   this.layoutForSelectRow.add(framework.fullNameWithKeyedAndVersion);
      //   }
      //   if (result.numberofCommits > 1) {
      //     this.moreThanOneCommit.add(framework.fullNameWithKeyedAndVersion);
      //   }
    ...
    print() {
      ...
      if (this.layoutForSelectRow.size > 0) {
        console.log("Warning: Implemenations that had a layout event for benchmarks that shouldn't layout (select1k):");
        for (let impl of this.layoutForSelectRow) {
          console.log(` ${impl}`);
        }
      } else {
        console.log("OK: No implemenation had a layout event for benchmarks that shouldn't layout (select1k):");
      }
      if (this.moreThanOneCommit.size > 0) {
        console.log("Warning: Implemenations that had a more than one (paint) commit event:");
        for (let impl of this.moreThanOneCommit) {
          console.log(` ${impl}, max delay between commit events was ${this.maxDeltaBetweenCommits.get(impl)} msecs`);
        }
      } else {
        console.log("OK: No implemenation had multiple (paint) commit events");
      }
