...
async function runBench(frameworkNames: string[]) {
  ...
  try {
    await page.goto(
      `http://${benchmarkOptions.host}:${benchmarkOptions.port}/csp/enable`,
    );
  } finally {
    await page.close();
    await browser.close();
  ...
  try {
    await page.goto(
      `http://${benchmarkOptions.host}:${benchmarkOptions.port}/csp`,
    );
    const extractedText = await page.$eval("*", (el: any) => el.innerText);
    console.log(extractedText);
    const failed = JSON.parse(extractedText);
    console.log(
      "CSP check failed for the following frameworks:\n",
      failed.join("\n"),
    );
  } finally {
    await page.close();
