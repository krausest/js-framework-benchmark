...
describe("duration is computed matching to some hand checked examples", () => {
  test("Ignore events from other thread", async () => {
    const result = await computeResultsCPU("unittests/cample-v3.1.0-keyed_07_create10k_3.json");
  ...
  test("Use the commit event after the layout event", async () => {
    const result = await computeResultsCPU("unittests/better-react-v1.1.3-keyed_01_run1k_1.json");
  ...
  test.only("Use the commit event after the fireAnimationFrame event", async () => {
    const result = await computeResultsCPU("unittests/dojo-v8.0.0-keyed_02_replace1k_0.json");
  ...
  test("Ignore timer with no following commit", async () => {
    const result = await computeResultsCPU("unittests/blazor-wasm-v7.0.1-keyed_01_run1k_11.json");
