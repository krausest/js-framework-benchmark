  ...
  render(ctx: IHeaderCtx, me: b.IBobrilNode) {
    ...
    me.children = divWithClass("row", [
      ...
      divWithClass("col-md-6", [
        divWithClass(
          "col-sm-6 smallpad",
          Button({ id: "run", action: () => store.run() }, "Create 1,000 rows")
        ),
        divWithClass(
          "col-sm-6 smallpad",
          Button(
            { id: "runlots", action: () => store.runLots() },
            "Create 10,000 rows"
          )
        ),
        divWithClass(
          "col-sm-6 smallpad",
          Button({ id: "add", action: () => store.add() }, "Append 1,000 rows")
        ),
        divWithClass(
          "col-sm-6 smallpad",
          Button(
            { id: "update", action: () => store.update() },
            "Update every 10th row"
          )
        ),
        divWithClass(
          "col-sm-6 smallpad",
          Button({ id: "clear", action: () => store.clear() }, "Clear")
        ),
        divWithClass(
          "col-sm-6 smallpad",
          Button(
...
const Row = b.createVirtualComponent<IRowData>({
  ...
  shouldChange(ctx: IRowCtx, me: b.IBobrilNode): boolean {
    return (
      ctx.data.item !== me.data.item || ctx.data.selected !== me.data.selected
    );
  },
