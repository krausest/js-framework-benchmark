...
export class AppComponent {
    ...
    buildData(count: number = 1000): Array<Data> {
        ...
        for (var i = 0; i < count; i++) {
            data.push({ id: this.id, label: adjectives[this._random(adjectives.length)] + " " + colours[this._random(colours.length)] + " " + nouns[this._random(nouns.length)], clazz: '' });
            this.id++;
        }
        return data;
    }
    ...
    select(item: Data, event: Event) {
        ...
        for (let i=0;i<this.data.length;i++) {
            if (this.data[i].clazz!=='') this.data[i].clazz = ''
        }
        item.clazz = 'danger'
    }
