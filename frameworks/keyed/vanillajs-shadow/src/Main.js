...
const styleLink = document.getElementById('root-style');
...
class Main {
    ...
    update() {
        this.store.update();
        for (let i=0;i<this.data.length;i+=10) {
            this.rows[i].shadowRoot.children[2].children[1].children[0].innerText = this.store.data[i].label;
        }
    }
    unselect() {
        if (this.selectedRow !== undefined) {
            this.selectedRow.shadowRoot.children[2].className = "";
            this.selectedRow = undefined;
        }
    }
    select(idx) {
