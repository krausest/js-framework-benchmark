<script>
  ...
  // nanostores atoms implement the Svelte store interface (subscribe returns unsubscribe),
  // so they can be used directly with the $ prefix in Svelte templates.
</script>

<div class="container">
  <div class="jumbotron">
    <div class="row">
      <div class="col-md-6"><h1>Astro-svelte-nanostores-keyed</h1></div>
      <div class="col-md-6">
        <div class="row">
    ...
    <tbody>
      {#each $rows as item (item.id)}
        <tr class={$selectedId === item.id ? "danger" : ""}
          ><td class="col-md-1">{item.id}</td><td class="col-md-4"
            ><a onclick={() => select(item.id)}>{item.label}</a></td
          ><td class="col-md-1"
            ><a onclick={() => removeRow(item.id)}><span class="glyphicon glyphicon-remove" aria-hidden="true" /></a
            ></td
