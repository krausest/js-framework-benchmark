...
class BenchmarkRow extends HTMLTableRowElement {
    ...
    // set label(label) {
    //     this.setAttribute('row-label', label);
    // }

    connectedCallback() {
        this.innerHTML = `
            <td class='col-md-1'>${this.dataId}</td>
            <td class='col-md-4'>
                <a class='lbl'>${this.rowLabel}</a>
            </td>
            <td class='col-md-1'>
...
class Main {
    ...
    createRow(data) {
        const tr = document.createElement('tr', { is: 'benchmark-row'});
        tr.dataId = data.id;
        tr.rowLabel = data.label;
        return tr;
    }
}
