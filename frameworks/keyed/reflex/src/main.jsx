const { h, render, syncState } = require("../node_modules/@zouloux/reflex/dist/reflex.es2017.min.js");
...
Row.isFactory = false
function Row ({ selected, item, dispatch, key, pure }, component) {
  component.shouldUpdate = (newProps, oldProps) => (oldProps.selected !== newProps.selected || oldProps.item !== newProps.item)
...
function reducer (reduceAction, initialState) {
  const currentState = syncState( initialState );
  return [
      currentState,
      async action => {
        // DO Synchronous changes
        currentState.value = reduceAction(currentState.value, action)
    ...
    <table className="table table-hover table-striped test-data">
      <tbody>
        {state.value.data.map(item => (
          <Row key={item.id} item={item} selected={state.value.selected === item.id} dispatch={dispatch} />
        ))}
      </tbody>
    </table>
