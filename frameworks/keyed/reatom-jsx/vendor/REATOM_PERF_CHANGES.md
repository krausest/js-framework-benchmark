# Reatom core + jsx performance changes

Base: `06a7f7a` (chore: run prettier)
Head: `d9e2e22` (fix(jsx,core): stop clear-memory leak from bind/reatomMap frames)

## Commits

- `d9e2e22` fix(jsx,core): stop clear-memory leak from bind/reatomMap frames
- `cdf4c36` fix(jsx): cleanup Text-node meta on unmount/clear
- `66bd49d` perf(jsx,core): skip empty atom names, iterator-free connect
- `0aab645` perf(jsx,core): cheaper class bindings, skip event naming, eager LL connect
- `07a6327` perf(jsx): text fast path, cheap fragment check, lighter on: handlers
- `661c225` perf: skip jsx resubscribe on DOM moves; faster clearLL

## Files changed

```
 packages/core/src/core/atom.ts                   |  13 +-
 packages/core/src/primitives/reatomLinkedList.ts |  56 ++-
 packages/jsx/src/index.test.tsx                  |  28 +-
 packages/jsx/src/index.ts                        | 418 ++++++++++++++++++-----
 packages/jsx/src/leaks.test.tsx                  | 123 +++++++
 packages/jsx/src/utils.ts                        |   2 +-
 6 files changed, 535 insertions(+), 105 deletions(-)
```

## Full diff

See [`reatom-core-jsx-perf.diff`](./reatom-core-jsx-perf.diff) for the complete patch.

```diff
diff --git a/packages/core/src/core/atom.ts b/packages/core/src/core/atom.ts
index ebdb9db..7a28552 100644
...
+  }
   ...
   if (setup.middlewares) {
     // @ts-expect-error
diff --git a/packages/core/src/primitives/reatomLinkedList.ts b/packages/core/src/primitives/reatomLinkedList.ts
index edc360b..56621ae 100644
...
@@ -76,6 +77,10 @@ export let DEBUG = atom(true, 'jsx.DEBUG')
...
+  }
   ...
   unlink(handle.start, () => {
diff --git a/packages/jsx/src/leaks.test.tsx b/packages/jsx/src/leaks.test.tsx
index d83989a..b8745ac 100644
--- a/packages/jsx/src/leaks.test.tsx
+++ b/packages/jsx/src/leaks.test.tsx
@@ -4,6 +4,7 @@ import {
   context,
   type Fn,
   isConnected,
+  reatomLinkedList,
   ...
   withInit,
   wrap,
@@ -102,3 +103,125 @@ test('atom child update after detach does not leak replaced content subscription
...
index 23f7355..abb2a32 100644
--- a/packages/jsx/src/utils.ts
